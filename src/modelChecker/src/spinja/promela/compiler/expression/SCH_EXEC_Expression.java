...
public class SCH_EXEC_Expression extends Expression implements CompoundExpression {
	 ...
	 */
	public SCH_EXEC_Expression(final Token token, final String id) {
		super(token);
		this.id = id;
		this.proc = null;
	...
	public SCH_EXEC_Expression(final Token token, final Proctype proc) {
		super(token);
	...
	public String getIntExpression() throws ParseException {
		...
		build.append("addProcess(new " + id + "_0(), target); } ") ;
