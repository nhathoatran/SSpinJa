...
public class ActionTransition extends Transition {
	...
	public boolean printTransitionImpl(StringWriter w) throws ParseException {
		...
		if (isComplex) {
			...
			for (Variable var : changes.keySet()) {
				if (Config.isCompileScheduler && 
						StringUtil.containSchedulerKeyword(var.toString())) continue ;
		...
		if (isComplex) {
			...
			if (Config.isCompileScheduler && sequence.containsSchedulerBehavior()) {
				w.appendLine("restoreModel(); ") ;
			} else
