...
public class Sequence extends Action implements ActionContainer {
	...
	public boolean containsSchedulerBehavior() {		
		for (Action act : actions) {
			if (StringUtil.containsSchedulerBehavior(act.toString()))
				return true ;
		}
		return false ;
	}
		...
		for(int i = actions.size() - 1; i >= 0; i--) {
			...
			if (generateBackup) {
				for (final Identifier id : a.getChangedVariables()) {
					if (StringUtil.containSchedulerKeyword(id.getIntExpression())) continue ;
					if (id.toString().contains("."))
						w.appendLine(id.toString(), " = _backup_", id.getVariable().getName(),
								";");
					else
	...
	public boolean getVarErrorAffect(String varName) {
		for (Action a : actions)
			if (a.getVarErrorAffect(varName))
				return true;
		return false;
	}
}
